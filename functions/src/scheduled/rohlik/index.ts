...
export const getCourierReportsAndSavePrague = functions
    ...
    .onRun(async (context) => {
      ...
      try {
        const data = await getReportsData(BevyRohlikCarriers.BEVY_PRAGUE, new Date().getTime());
...
export const getCourierReportsAndSavePlzen = functions
    ...
    .onRun(async (context) => {
      ...
      try {
        const data = await getReportsData(BevyRohlikCarriers.BEVY_PLZEN, new Date().getTime());
