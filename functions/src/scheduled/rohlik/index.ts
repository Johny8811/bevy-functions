...
export const getCourierReportsAndSavePrague = functions
    ...
    .onRun(async (context) => {
      ...
      try {
        const data = await getReportsData(BevyRohlikCarriers.BEVY_PRAGUE, Number(context.timestamp));
...
export const getCourierReportsAndSavePlzen = functions
    ...
    .onRun(async (context) => {
      ...
      try {
        const data = await getReportsData(BevyRohlikCarriers.BEVY_PLZEN, Number(context.timestamp));
